import React from "react";

function Adventure() {
  return (
    <div>
      

      {/* featured Placeses section */}
      <div className="featured-Places">
        <h2>Featured Placeses</h2>
        <div className="card-container">
          <div className="Places-card">
            <img src="https://www.example.com/beach1.jpg" alt="Places1" />
            <h3>Places1</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach2.jpg" alt="Places2" />
            <h3>Places2</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places3" />
            <h3>Places3</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places4" />
            <h3>Places4</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places5" />
            <h3>Places5</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places6" />
            <h3>Places6</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
        </div>
        <a href="/sustainability" className="view-more-btn">
          View More
        </a>
      </div>

      {/* family-friendly beaches section */}
      <div className="beach-category">
        <h2>Family-friendly beaches</h2>
        {/* add your family-friendly beaches here */}
        <div className="card-container">
          <div className="Places-card">
            <img src="https://www.example.com/beach1.jpg" alt="Places1" />
            <h3>Places1</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach2.jpg" alt="Places2" />
            <h3>Places2</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places3" />
            <h3>Places3</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places4" />
            <h3>Places4</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places5" />
            <h3>Places5</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places6" />
            <h3>Places6</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
        </div>
        <a href="/sustainability" className="view-more-btn">
          View More
        </a>
      
      </div>

      {/* romantic beaches section */}
      <div className="beach-category">
        <h2>Romantic beaches</h2>
        {/* add your romantic beaches here */}
        <div className="card-container">
          <div className="Places-card">
            <img src="https://www.example.com/beach1.jpg" alt="Places1" />
            <h3>Places1</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach2.jpg" alt="Places2" />
            <h3>Places2</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places3" />
            <h3>Places3</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places4" />
            <h3>Places4</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places5" />
            <h3>Places5</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach3.jpg" alt="Places6" />
            <h3>Places6</h3>
            <p>
              Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="/">Learn More</a>
          </div>
        </div>
        <a href="/sustainability" className="view-more-btn">
          View More
        </a>
      
      </div>

      {/* adventure beaches section */}
      <div className="beach-category">
        <h2>Adventure beaches</h2>
        {/* add your adventure beaches here */}
        <div className="card-container">
          <div className="Places-card">
            <img src="https://www.example.com/beach1.jpg" alt="Places1" />
            <h3>Places1</h3>
            <p>
               amanisjjdjj jajnsjd sjd jnasjans a sajnja js ja 
              s ja sj as hja smjasmjsbnjx ajnajbsjamsj asamsjnasjajsja s ajjj
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.example.com/beach2.jpg" alt="Places2" />
            <h3>Places2</h3>
            <p>
               amanisjjdjj jajnsjd sjd jnasjans a sajnja js ja 
              s ja sj as hja smjasmjsbnjx ajnajbsjamsj asamsjnasjajsja s ajjj
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.trawell.in/admin/images/thumbs/18383177Indore_Mahatma_Gandhi_Hall_Main_thumb.jpg" alt="Places3" />
            <h3>Places3</h3>
            <p>
               amanisjjdjj jajnsjd sjd jnasjans a sajnja js ja 
              s ja sj as hja smjasmjsbnjx ajnajbsjamsj asamsjnasjajsja s ajjj
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.trawell.in/admin/images/upload/094484563Indore_Rajwada_Palace_Main.jpg" alt="Places4" />
            <h3>Places4</h3>
            <p>
               amanisjjdjj jajnsjd sjd jnasjans a sajnja js ja 
              s ja sj as hja smjasmjsbnjx ajnajbsjamsj asamsjnasjajsja s ajjj
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.trawell.in/admin/images/thumbs/183831995Lalbagh_thumb.jpg" alt="Places5" />
            <h3>Places5</h3>
            <p>
              amanisjjdjj jajnsjd sjd jnasjans a sajnja js ja 
              s ja sj as hja smjasmjsbnjx ajnajbsjamsj asamsjnasjajsja s ajjj
            </p>
            <a href="/">Learn More</a>
          </div>
          <div className="Places-card">
            <img src="https://www.trawell.in/admin/images/thumbs/183831687Indore_Central_Museum_Main_thumb.jpg" alt="Places6" />
            <h3>Places6</h3>
            <p>
               amanisjjdjj jajnsjd sjd jnasjans a sajnja js ja 
              s ja sj as hja smjasmjsbnjx ajnajbsjamsj asamsjnasjajsja s ajjj
            </p>
            <a href="/">Learn More</a>
          </div>
        </div>
        <a href="/sustainability" className="view-more-btn">
          View More
        </a>
      
      </div>
    </div>


  );
}

export default Adventure;
